...
class filterScan : public edm::one::EDAnalyzer<edm::one::SharedResources>  {

public:
  explicit filterScan(const edm::ParameterSet&);
  ~filterScan();  
...
private:
  ...
  edm::EDGetTokenT<CSCCorrelatedLCTDigiCollection> theCorrelatedLCTDigiToken;
  edm::EDGetTokenT<CSCCLCTDigiCollection> CLCTToken;
  edm::EDGetTokenT<CSCRecHit2DCollection> theCSCRecHitToken;
  edm::EDGetTokenT<CSCComparatorDigiCollection> ComparatorToken;
  ...
  int evtsProceeded2D; // event counter 2D hits
  ...
  bool handleCLCT  (const edm::Handle<CSCCLCTDigiCollection> clcts);
  bool handle2Dhits  (const edm::Handle<CSCRecHit2DCollection> recHits);
  bool handle2DLCT  (const edm::Handle<CSCCorrelatedLCTDigiCollection> corrlctDigiTag);
  bool handleComparator  (const edm::Handle<CSCComparatorDigiCollection> comparator);
  bool handleStrips  (const edm::Handle<CSCStripDigiCollection> strips);
  ...
  TH1F  * hChitL[6][1];    // cathode hits from strips: [0] - all hits
  ...
  TH2F * Chamber2RecHits[6];
  TH2F * Chamber2LCTHits;
...
filterScan::filterScan(const edm::ParameterSet& iConfig):
		evtsProceededA(0),
		evtsProceeded2D(0),
		evtsProceededS(0),
		prevEvtN(0),
   ...
   edm::InputTag clctDigiTag  = iConfig.getParameter<edm::InputTag>("clctDigiTag");
   CLCTToken = consumes<CSCCLCTDigiCollection>(clctDigiTag);
   
   edm::InputTag compDigiTag  = iConfig.getParameter<edm::InputTag>("compDigiTag");
   ComparatorToken = consumes<CSCComparatorDigiCollection>(compDigiTag);
   
   edm::InputTag stripDigiTag  = iConfig.getParameter<edm::InputTag>("stripDigiTag");
   StripToken = consumes<CSCStripDigiCollection>(stripDigiTag);


   cscRecHitTag = iConfig.getUntrackedParameter<edm::InputTag>("cscRecHitTag");
   cscSegTag    = iConfig.getUntrackedParameter<edm::InputTag>("cscSegTag");
   ...
   theCSCRecHitToken = consumes<CSCRecHit2DCollection>(cscRecHitTag);
...
void filterScan::beginJob(){
  ...
  if(!anodeOnlyTag){
    ...
    Chamber2LCTHits      = new TH2F("Chamber2LCTHits",     "LCT;wg [cm];halfstrip [cm]", 126, -5.5, 120.5 ,146, -5.5, 140.5);
  };
  ...
  for(short unsigned int i=0; i<6; i++){
    ...
    if(!anodeOnlyTag){
      ...
      hname = "Chamber2RecHits[";hname+=i; hname+="]"; Chamber2RecHits[i] = new TH2F(hname.Data(), hname.Data(), 120, -70., 50., 240, -120., 120.);
      fout->cd("CLCT");
      hname = "hChitL[";      hname+=i; hname+="]["; hname+=0; hname+="]"; hChitL[i][0]    = new TH1F(hname.Data(), hname.Data(), 112, 0, 112);
	  };
  };
...
void filterScan::endJob() {
  ...
  std::cout << "2D hit events: " << evtsProceeded2D << std::endl;
}
...
* analysing                                          *
******************************************************/
void filterScan::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
...
//	  std::cout << "\n\n================================================\n-=\t"<< Event << "\t=-\n";
  ...
  if(!anodeOnlyTag){
    ...
    bool fullEvtLCT = handle2DLCT(corrlctDigiTag);
    
    edm::Handle<CSCRecHit2DCollection> recHits;
    iEvent.getByToken(theCSCRecHitToken, recHits);
    //std::cout << " RecHits size is: "<<recHits->size()<< std::endl;
    bool fullEvt2D = handle2Dhits(recHits);
    if(fullEvt) {
          evtsProceeded2D++;
    }
    ...
    //bool fullEvtCLCT = handleCLCT(clctDigiTag);
    
    //edm::Handle<CSCComparatorDigiCollection> compDigiTag;
    //iEvent.getByToken(ComparatorToken, compDigiTag);
    //bool fullEvtComp = handleComparator(compDigiTag);
    
    edm::Handle<CSCStripDigiCollection> stripDigiTag;
    iEvent.getByToken(StripToken, stripDigiTag);
    bool fullEvtStrip = handleStrips(stripDigiTag);
...
*****************************************************************************************************/
bool filterScan::handleAnodes(const edm::Handle<CSCWireDigiCollection> wires, const edm::Handle<CSCALCTDigiCollection> alct){
...
//===========================================================================
//===========================================================================
bool filterScan::handleComparator(const edm::Handle<CSCComparatorDigiCollection> comparator) {
//===========================================================================
  
  int count = 0;
  CSCComparatorDigiCollection::DigiRangeIterator detUnitIt;
  /*
  for (detUnitIt = comparator.begin(); detUnitIt != comparator.end(); ++detUnitIt) {
    ...
    for (CSCComparatorDigiCollection::const_iterator digiIt = range.first; digiIt != range.second; ++digiIt) {
      ...
      bool digi_valid = (*digiIt).isValid();
      if (digi_valid) {
    		int layer_no = id.layer();
			  int strip_no = (*digiIt).getStrip();// halfstrip # what does getKeyStrip do?
			  hChitL[layer_no-1][0]->Fill(strip_no);
			  count++;
      }
...
//===========================================================================
//===========================================================================
bool filterScan::handleStrips(const edm::Handle<CSCStripDigiCollection> strips) {
//===========================================================================
  
  int count = 0;
  CSCStripDigiCollection::DigiRangeIterator detUnitIt;
  /*
  for (detUnitIt = strips.begin(); detUnitIt != strips.end(); ++detUnitIt) {
...
//===========================================================================
//===========================================================================
bool filterScan::handleCLCT(const edm::Handle<CSCCLCTDigiCollection> clcts) {
//===========================================================================
  ...
  for (detUnitIt = clcts->begin(); detUnitIt != clcts->end(); detUnitIt++) {
    ...
    for (CSCCLCTDigiCollection::const_iterator digiIt = range.first; digiIt != range.second; digiIt++) {
      ...
      if (clct_valid) {
    		...
    		int keystrip  = (*digiIt).getKeyStrip(); // halfstrip # what does getStrip do?
...
//===========================================================================
//===========================================================================
bool filterScan::handle2Dhits  (const edm::Handle<CSCRecHit2DCollection> rechithandle){
//===========================================================================
	...
	for(CSCRecHit2DCollection::const_iterator rechit=rechithandle->begin(); rechit != rechithandle->end(); ++rechit){
	    CSCDetId idrec = (*rechit).cscDetId();

	    if(debug){
	      std::cout <<  "2Dhits: chamber " << idrec.chamber()
			<< " station " << idrec.station()
			<< " ring    " << idrec.ring()
			<< " layer   " << idrec.layer() << std::endl;
	    };
      ...
      // Fill histos
      if(kChamber==2){
        Chamber2RecHits[kLayer-1]->Fill(xreco, yreco);
      }
...
//===========================================================================
//===========================================================================
bool filterScan::handle2DLCT  (const edm::Handle<CSCCorrelatedLCTDigiCollection> lcthandle){
//===========================================================================
	//int hitsPerLayer[6] = {0,0,0,0,0,0};
	for (CSCCorrelatedLCTDigiCollection::DigiRangeIterator j = lcthandle->begin(); j != lcthandle->end(); j++) {
      ...
      for (CSCCorrelatedLCTDigiCollection::const_iterator digiIt = range.first; digiIt != range.second; ++digiIt) {
        ...
        if ((*digiIt).isValid()) {
          ...
          float ycoord=(*digiIt).getStrip();
          float xcoord=(*digiIt).getKeyWG();
          ...
          if(kChamber==2){
            Chamber2LCTHits->Fill(xcoord, ycoord);
            //if(ycoord<128&&ycoord>-0.5){
...
//===========================================================================
//===========================================================================
int filterScan::handleSegments(const edm::Handle<CSCSegmentCollection> segments){
//===========================================================================
...
void filterScan::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
