...
parser.add_argument('--startdate', action='store', default="01 SEP 2017", type=str, help="Starting date: write as DD MTH YYYY where MTH is the 3 letter abbreviation for the month\n DEFAULT: 01 SEP 2017")
...
if args.test11compare:
    ...
    TMBRateCanvas.cd()
    if args.timeplot:
        TMBRateLimits = Limits("TMBRatelimits", "Dark Rates", "Date of Measurement", "Dark Rate (kHz)", 1000, tmin-fivedays, tmax+fivedays, 100, 0, 2)
    elif args.validatewithold:
        TMBRateLimits = Limits("TMBRatelimits", "TMB Dump Dark Rates", "Accumulated Charge (mC/cm)", "Dark Rate (kHz)", 1000, 0, 500, 100, 0, 2)
    else:
        TMBRateLimits = Limits("TMBRatelimits", "TMB Dump Dark Rates", "Accumulated Charge (mC/cm)", "Dark Rate (kHz)", 1000, 330, qmax + 10, 100, 0, 2)
    plotALCTrates.draw()
